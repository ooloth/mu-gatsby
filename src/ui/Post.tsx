...
const findDevToYouTubeEmbed = (html: string): RegExpMatchArray | null =>
  html.match(/<p>\{% youtube .+ %\}<\/p>/i)

const replaceDevToEmbedsWithIframes = (html: string): string => {
  const devToYouTubeEmbed = findDevToYouTubeEmbed(html)

  if (!devToYouTubeEmbed) {
    return html
  }

  const embedCode = devToYouTubeEmbed[0].substring(14, 25)

  const youTubeIframe = `
    <div data-id="iframe-container">
      <iframe width="560" height="315" src="https://www.youtube.com/embed/${embedCode}" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen>
      </iframe>
    </div>`

  return html.replace(devToYouTubeEmbed[0], youTubeIframe)
}

const Post = ({ data: { devArticle: article } }: { data: any }) => {
  const metadata = getPostMetadata(article)

  const htmlWithoutDevToEmbeds = replaceDevToEmbedsWithIframes(
    ...
    <Base>
      ...
      <Main>
        <article>
          ...
          <Body dangerouslySetInnerHTML={{ __html: htmlWithoutDevToEmbeds }} />
